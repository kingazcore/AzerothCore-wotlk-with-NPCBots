...
local function GetPlayerInfo(player)--得到玩家信息
	...
	return string.format("来自%s的%s玩家[|cFF00FF00|Hplayer:%s|h%s|h|r]",Pteam,Pclass,Pname,Pname)
end

local function PlayerFirstLogin(event, player)--玩家首次登录
	SendWorldMessage("|cFFFF0000只听一声惊雷炸响，|r"..GetPlayerInfo(player).." |cFFFF0000来到了这个充满纷争的世界上。|r")
	print("Player is Created. GUID:"..player:GetGUIDLow())
end

local function PlayerLogin(event, player)--玩家登录
	SendWorldMessage("|cFFFF0000[上线通知]|r"..GetPlayerInfo(player).." |cFFFF0000上线了。|r")
